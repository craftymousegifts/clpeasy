...
const server=http.createServer((req,res)=>{let f=decodeURIComponent(req.url.split('?')[0]);if(f==='/')f='/index.html';const p=path.join(ROOT,f);
 ...
 const saveRes=await b.t.evaluate(async()=>{document.getElementById('scent-name').value='QA Lavender';try{await saveLabel();}catch(e){return 'threw '+e.message}
   await new Promise(r=>setTimeout(r,500));return {editingLabelId, keys:Object.keys(localStorage), saved:localStorage.getItem('clpeasy_labels__u_u1')&&JSON.parse(localStorage.getItem('clpeasy_labels__u_u1')).map(e=>e.scentName+'|'+e.id)};});
 console.log('BUILDER', JSON.stringify(saveRes), b.errs);
 const pr=await open('print.html');
 console.log('COMPOSER', await pr.t.evaluate(()=>document.getElementById('saved-list').innerText.replace(/\s+/g,' ').slice(0,200)), pr.errs);
 const ml=await open('my-labels.html');
 console.log('MYLABELS', await ml.t.evaluate(()=>({url:location.href,grid:document.getElementById('grid-area').innerText.replace(/\s+/g,' ').slice(0,200),tabs:document.getElementById('folder-tabs').innerText})), ml.errs);
