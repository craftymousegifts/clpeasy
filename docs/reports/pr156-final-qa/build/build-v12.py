# Builds the v12 isolated PR #156 QA site from the committed PR head.
import os, re, shutil, subprocess, json
REPO='/home/user/clpeasy'
import sys
S=sys.argv[1] if len(sys.argv)>1 else os.environ['CLP_BUILD_SCRATCH']  # holds 'testanon' (CLPeasy Test public anon key)
OUT=S+'/v12'
...
open(OUT+'/TEST-ENVIRONMENT.txt','w').write(f"CLPeasy PR #156 isolated test candidate v12 (commit {head}), deployed to the v10 test site.\nSupabase: CLPeasy Test ({TEST_REF}). Stripe: Sandbox (acct_1TdczqKF3jvQfgEa). Not production.\n")
json.dump(report, open(S+'/v12-report.json','w'), indent=1)
