...
const E = require('/home/user/clpeasy/entitlement.js');

const ROOT = '/home/user/clpeasy';
const DB = 'clpeasy_b1_patch_test';
...
function freshDb(migrations){
  ...
  run(fs.readFileSync('/home/user/clpeasy/tests/sql/supabase-shim.sql','utf8'));
  for (const m of migrations) run(fs.readFileSync(path.join(ROOT,'supabase','migrations',m),'utf8'));
  if (migrations.length >= 3) run(fs.readFileSync('/home/user/clpeasy/docs/reports/pr156-final-qa/proposed-fixes/B1-redownload-after-upgrade.sql','utf8'));
}
...
// A live-trial watermarked re-download stays watermarked even after buying PAYG.
{
  ...
  assert.deepStrictEqual([again.free_redownload, again.consumed, again.clean_export], [false, true, true], 'PROPOSED B1: after buying downloads, the trial-watermarked label is charged once and exported clean');
  assert.strictEqual(profile(id).topup_credits, 4, 'PROPOSED B1: exactly one purchased download spent');
  const third = consumeAs(id, 'rose::scented candle');
  assert.deepStrictEqual([third.free_redownload, third.clean_export, profile(id).topup_credits], [true, true, 4], 'PROPOSED B1: then free and clean for 7 days');
  const noCredit = makeUser({ plan:'free', subscription_status:'trialing', trial_end:iso(3*DAY), downloads_limit:10, downloads_used:0, topup_credits:0 });
  consumeAs(noCredit, 'iris::scented candle');
  const trialAgain = consumeAs(noCredit, 'iris::scented candle');
  assert.deepStrictEqual([trialAgain.free_redownload, trialAgain.clean_export, profile(noCredit).downloads_used], [true, false, 1], 'PROPOSED B1: trial with no clean entitlement keeps the free watermarked re-download');
  const zero = makeUser({ plan:'payg', subscription_status:'payg', trial_end:iso(-DAY), downloads_limit:0, downloads_used:0, topup_credits:0 });
  run(`insert into public.label_downloads(user_id,label_key,last_downloaded_at,clean_export) values(${q(zero)},'old::candle',now(),false);`);
  const z = consumeAs(zero, 'old::candle');
  assert.deepStrictEqual([z.ok, z.free_redownload, z.clean_export], [true, true, false], 'PROPOSED B1: no clean download available -> free watermarked copy, never blocked');
  console.log('PASS: PROPOSED B1 scenarios (4)');
  checks += 3;
}
...
// ── Annual plans refill monthly (pricing: "£99/year · 20 downloads/month") ──
{
  ...
  const E2 = require('/home/user/clpeasy/entitlement.js');
